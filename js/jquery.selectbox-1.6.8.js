 ...
 * 
 * Changelog :
 * Version 1.6.8 by Andre Hayter
 * - removed extra click on selected option to fix ie6 bug
 * Version 1.6.7 by Andre Hayter
 * - changed <select> to be "visibility: hidden" rather than "display: none" so that validation will still work
 * Version 1.6.6 by Andre Hayter
 * - fixed a FF bug where removing Attr caused the val to not update - now sets prop to ''
 ...
 * - fixed refreshing of selectbox
 * Version 1.6.2 by Andre Hayter
 * - fixed a selection issue when refreshing via postback
...
jQuery.SelectBox = function (selectobj, options, index) {
  ...
  opt.matchWidth = opt.matchWidth || true;
  ...
  $input
	...
	.focus(function () {
	  if ($ddWrapper.not(':visible') && !disabled) {
	    inFocus = true;
	    $ddWrapper.show();
	    setHeight();
	    setWidth();
